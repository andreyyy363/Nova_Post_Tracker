...
export class PackageService {
  ...
  static validateTrackingNumber(trackingNumber: string): {
    ...
    if (/^\d{14}$/.test(cleanNumber)) {
      ...
      return {
        isValid: false,
        message:
          'Невірний формат номера посилки. Має бути 14 цифр для внутрішньої або 20 цифр для міжнародної відправки.',
      };
    }
  }
  ...
  static async validatePackageAccess(trackingNumber: string): Promise<{
    ...
    if (!isUserPackage) {
      return {
        authorized: false,
        message:
          'Цей номер посилки не належить до списку ваших посилок. Додайте посилку, щоб переглянути деталі.',
        cleanNumber,
      };
    }
  ...
  static async savePackageToStorage(packageData: TrackingData): Promise<{
    ...
    try {
      ...
      return {
        success: false,
        message: 'Виникла помилка при збереженні посилки',
      };
    }
  }
  ...
  static async getAllSavedPackages(): Promise<TrackingData[]> {
    try {
      const savedPackagesStr = await AsyncStorage.getItem(
        this.ALL_PACKAGES_KEY,
      );
      if (savedPackagesStr) {
  ...
  static async removePackage(
    ...
    try {
      if (!packageNumber) {
        return {
          success: false,
          message: 'Не вказано номер посилки для видалення',
        };
      }
      ...
      const existingPackage = savedPackages.find(
        pkg => pkg.Number === cleanNumber,
      );
      if (!existingPackage) {
        return {
          success: false,
          message: 'Посилка з таким номером не знайдена в списку',
        };
      }
  ...
  static isPackageInList(
    packageNumber: string,
    packages: TrackingData[],
  ): boolean {
